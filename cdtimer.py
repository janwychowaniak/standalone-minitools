...
if __name__ == '__main__':
	...
	if len(sys.argv) < 3:
		print 'Usage:'
		print '  ' + sys.argv[0] + '   time(HH:MM:SS)   intention'
		print
#		printhelp()
		sys.exit(1)

	appname = sys.argv[0]
	remtime_s = sys.argv[1]
	intention_list = sys.argv[2:]
	intention = ' '.join(intention_list)
	...
	#~ while True:
		...
		#~ if   key == "z" or key == "Z":
			...
			#~ rwrite('['+remaining_time.get_time()+']')
		#~ elif key == "x" or key == "X":
			#~ remaining_time.add30()
			#~ rwrite('['+remaining_time.get_time()+']')
		#~ elif key == "M":
			#~ remaining_time.add1s()
			#~ rwrite('['+remaining_time.get_time()+']')
		#~ elif key == "c" or key == "C":
			#~ break
	...
	def signal_handler(signal, frame):
		...
		print 'Ctrl+C!'
		print 
		print appname + ' ' + timer.get_time() + ' ' + intention
		timer.stop()
		timer.join()
		sys.exit(2)
